...
const [packageJsonPath, deployPath, packageJsonExists, deployExists] =
  getPaths();

let params = { dir: path.dirname(packageJsonPath) };
if (!packageJsonExists && !deployExists)
  exit(
    `Could not locate package.json nor .deploy-installer at path: ${path.dirname(
      packageJsonPath
    )}`
  );
if (packageJsonExists)
  params = { ...params, ...parsePackageJson(packageJsonPath) };
...
writeFile(
  path.join(path.dirname(packageJsonPath), "win-install", installerName),
  installerParams + bundlePs(path.join(__dirname, "main.ps1"))
);
exit();
